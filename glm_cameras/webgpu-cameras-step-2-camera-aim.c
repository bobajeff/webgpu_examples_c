...
void cameraAim(float * eye, float * target, float * up, mat4 dest){
    vec3 tmp_vector;
    vec3 zAxis;
    glm_vec3_sub(eye, target, tmp_vector);
...
int main(int argc, char *argv[]) {
  ...
  while (!glfwWindowShouldClose(window)) {
   ...
   WGPUTextureView depthStencilAttachment_view = wgpuTextureCreateView(depthTexture, NULL);
    ...
    glm_translate_to(tmp_matrix_2, (float[3]){0, 0, radius * 1.5}, tmp_matrix_3);

    // Get the camera's position from the matrix we computed
    float eye[3] = {tmp_matrix_3[3][0], tmp_matrix_3[3][1], tmp_matrix_3[3][2]};

    float up[3] = {0, 1, 0};

    // Compute the camera's matrix using look at.
    mat4 cameraMatrix;
    cameraAim(eye, fPosition, up, cameraMatrix);
