...
int main(int argc, char *argv[]) {
  ...
  while (!glfwWindowShouldClose(window)) {
   ...
   WGPUTextureView depthStencilAttachment_view = wgpuTextureCreateView(depthTexture, NULL);
    ...
    glm_rotate_y(tmp_matrix, degToRad(cameraAngle), cameraMatrix);
    
    glm_translate_to(cameraMatrix, (float[3]){0, 0, radius * 1.5}, tmp_matrix);

    // Make a view matrix from the camera matrix.
    mat4 viewMatrix;
    glm_mat4_inv(tmp_matrix, viewMatrix);
