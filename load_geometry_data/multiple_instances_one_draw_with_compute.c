...
int main(int argc, char *argv[]) {
  ...
  WGPUShaderModuleDescriptor computeShaderSource =
      load_wgsl(RESOURCE_DIR "compute.wgsl");
...
#define numFs 10
  ...
  float *meshData;
  int meshDataDataSize;
  long numVerticesMesh;
  get_vertex_data(RESOURCE_DIR "f_vertex_data_flipped_centered.ply", &meshData,
                  &meshDataDataSize, &numVerticesMesh);
  long numVertices = numVerticesMesh * numFs;
  ...
  wgpuQueueWriteBuffer(queue, meshBuffer, 0, meshData, meshDataDataSize);
  ...
  // set xz coords of first F
  float f_xz_coords[numFs][2];
  float angle = ((float)0 / numFs) * M_PI * 2;
  f_xz_coords[0][0] = cosf(angle) * radius;
  f_xz_coords[0][1] = sinf(angle) * radius;
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    wgpuComputePassEncoderSetBindGroup(compute_pass, 0, computeBindGroup, 0,
                                       NULL);
    wgpuComputePassEncoderDispatchWorkgroups(compute_pass, numVerticesMesh,
                                             numFs, 1);
