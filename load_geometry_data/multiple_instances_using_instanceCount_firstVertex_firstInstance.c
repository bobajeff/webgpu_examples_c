...
int main(int argc, char *argv[]) {
  ...
  WGPUShaderModuleDescriptor shaderSource =
      load_wgsl(RESOURCE_DIR "multiple_instances_using_instanceCount.wgsl");
...
#define numMeshes 3
  ...
  // calculate instance offsets and xz_coords_size
  int instance_offsets[numMeshes];
  int instance_offset = 0;
  int xz_coords_size = 0;
  for (i = 0; i < numMeshes; i++) {
    instance_offsets[i] = instance_offset;
    xz_coords_size += mesh_xz_coords_size[i];
    instance_offset += mesh_instances[i];
  };
  // combine xz_coords into single array
  float *xz_coords = (float *)malloc(xz_coords_size);
  for (i = 0; i < numMeshes; i++) {
    memcpy(&xz_coords[instance_offsets[i] * 2], mesh_xz_coords[i],
           mesh_xz_coords_size[i]);
    free(mesh_xz_coords[i]);
  };
  ...
  WGPUBuffer staticStorageBuffer = wgpuDeviceCreateBuffer(
      device, &(WGPUBufferDescriptor){.label = "static storage for objects",
                                      .size = xz_coords_size,
                                      .usage = WGPUBufferUsage_Storage |
                                               WGPUBufferUsage_CopyDst,
                                      .mappedAtCreation = false});
  wgpuQueueWriteBuffer(queue, staticStorageBuffer, 0, xz_coords,
                       xz_coords_size);
  ...
  // calculate offsets and vertexDataSize
  int offsets[numMeshes];
  int offset = 0;
  for (i = 0; i < numMeshes; i++) {
    offsets[i] = offset;
    vertexDataSize += _vertexDataSize[i];
    offset += _numVertices[i];
  };
  // combine vertexData in single array
  vertexData = (float *)malloc(vertexDataSize);
  for (i = 0; i < numMeshes; i++) {
    memcpy(&vertexData[offsets[i] * 4], _vertexData[i], _vertexDataSize[i]);
    free(_vertexData[i]);
  };
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    float fPosition[3] = {xz_coords[0], 0, xz_coords[1]};
    ...
    for (i = 0; i < numMeshes; i++) {
      ...
      wgpuRenderPassEncoderDraw(pass, _numVertices[i], numInstances[i],
                                offsets[i], instance_offsets[i]);
    }
