...
int main(int argc, char *argv[]) {
  ...
  WGPUShaderModuleDescriptor shaderSource = load_wgsl(
      RESOURCE_DIR "multiple_instances_using_instanceCount_and_StepModeInstance.wgsl");
...
#define numFs 10
  ...
  printf("-----------------\n");
  const uint64_t instanceBufferSize = sizeof f_xz_coords;
  WGPUBuffer instanceBuffer = wgpuDeviceCreateBuffer(
    device, &(WGPUBufferDescriptor){.label = "instance buffer",
                                    .size = instanceBufferSize,
                                    .usage = WGPUBufferUsage_Vertex |
                                            WGPUBufferUsage_CopyDst,
                                    .mappedAtCreation = false});
    wgpuQueueWriteBuffer(queue, instanceBuffer, 0, f_xz_coords,
                       sizeof f_xz_coords);
  ...
  int vertexDataSize;
  long numVertices;
  get_vertex_data(RESOURCE_DIR "f_vertex_data_flipped_centered.ply", &vertexData, &vertexDataSize, &numVertices);
  ...
  while (!glfwWindowShouldClose(window)) {
   ...
   WGPUTextureView depthStencilAttachment_view = wgpuTextureCreateView(depthTexture, NULL);
    ...
    wgpuRenderPassEncoderDraw(pass, numVertices, numFs, 0, 0);
