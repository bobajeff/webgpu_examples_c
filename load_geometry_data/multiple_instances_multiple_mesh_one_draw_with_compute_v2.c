...
typedef struct PartMap {
  u_int32_t meshMapIndex;
  u_int32_t numFacesInPart;
} PartMap;

typedef struct MeshMap {
...
int main(int argc, char *argv[]) {
  ...
  WGPUShaderModuleDescriptor compute_shader_source =
      load_wgsl(RESOURCE_DIR
                "multiple_instances_multiple_mesh_one_draw_with_compute_v2.wgsl");
  ...
  for (i = 0; i < NUM_MESHES; i++) {
    ...
    // set mesh map data
    mesh_maps[i].numMeshVertices = raw_num_mesh_vertices[i];
    mesh_maps[i].outMeshVertexOffset = vertex_count;
    mesh_maps[i].numMeshInstances = num_mesh_instances[i];
  ...
  float *mesh_bundle = (float *)malloc(mesh_bundle_size);
  ...
  int vertex_data_size = vertex_count * 4 * sizeof(float);
  ...
  WGPUBuffer mesh_map_buffer = wgpuDeviceCreateBuffer(
      device, &(WGPUBufferDescriptor){
                  .nextInChain = NULL,
                  .label = "part buffer",
                  .usage = WGPUBufferUsage_Storage | WGPUBufferUsage_CopyDst,
  ...
  wgpuQueueWriteBuffer(queue, part_buffer, 0, part_maps, part_maps_size);
  wgpuQueueWriteBuffer(queue, mesh_map_buffer, 0, mesh_maps, mesh_maps_size);
      ...
      &(WGPUBindGroupDescriptor){
          ...
          .entries = (WGPUBindGroupEntry[]){{.nextInChain = NULL,
                                            ...
                                            {.nextInChain = NULL,
                                             ...
                                             {.nextInChain = NULL,
                                             .binding = 4,
                                             .buffer = mesh_map_buffer,
                                             .offset = 0,
                                             .size = mesh_maps_size}},
          .entryCount = 5, //**Important!**
      });
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    wgpuComputePassEncoderSetBindGroup(compute_pass, 0, compute_bind_group, 0,
                                       NULL);
    wgpuComputePassEncoderDispatchWorkgroups(compute_pass, parts_count,
