...
int main(int argc, char *argv[]) {
  ...
  WGPUShaderModuleDescriptor shaderSource = load_wgsl(
      RESOURCE_DIR "multiple_instances_using_instanceCount.wgsl");
  ...
  // calculate size of staticStorageBuffer
  for (i = 0; i < numMeshes; i++) {
  ...
  for (i = 0; i < numScreenObjects; i++) {
    ...
    printf("%f %f\n", xz_coords[mesh_instances[mesh_index] * 2], xz_coords[mesh_instances[mesh_index] * 2 + 1] );
    mesh_instances[mesh_index]++;
  };
  printf("-----------------\n");

  float * vertexData[numMeshes];
  int vertexDataSize[numMeshes];
  long numVertices[numMeshes];
  char * ply_file_paths[numMeshes] = {RESOURCE_DIR "f_vertex_data_flipped_centered.ply", RESOURCE_DIR "B.ply", RESOURCE_DIR "b.ply"};
  WGPUBuffer vertexBuffer[numMeshes];

  WGPUBuffer staticStorageBuffer[numMeshes];
  WGPUBuffer uniformBuffer[numMeshes];
  WGPUBindGroup bindGroup[numMeshes];

  for (i = 0; i < numMeshes; i++) {
    // Open ply file, and read into vertexData
    get_vertex_data(ply_file_paths[i], &vertexData[i], &vertexDataSize[i],
                    &numVertices[i]);
    vertexBuffer[i] = wgpuDeviceCreateBuffer(
        device, &(WGPUBufferDescriptor){.label = "vertex buffer vertices",
                                        .size = vertexDataSize[i],
                                        .usage = WGPUBufferUsage_Vertex |
                                                 WGPUBufferUsage_CopyDst,
                                        .mappedAtCreation = false});
    wgpuQueueWriteBuffer(queue, vertexBuffer[i], 0, vertexData[i],

                         vertexDataSize[i]);
    staticStorageBuffer[i] = wgpuDeviceCreateBuffer(
        device, &(WGPUBufferDescriptor){.label = "static storage for objects",
                                        .size = mesh_xz_coords_size[i],
                                        .usage = WGPUBufferUsage_Storage |
                                                 WGPUBufferUsage_CopyDst,
                                        .mappedAtCreation = false});
    wgpuQueueWriteBuffer(queue, staticStorageBuffer[i], 0, mesh_xz_coords[i],
                         mesh_xz_coords_size[i]);
  ...
  while (!glfwWindowShouldClose(window)) {
   ...
   WGPUTextureView depthStencilAttachment_view = wgpuTextureCreateView(depthTexture, NULL);
    ...
    for (i = 0; i < numMeshes; i++) {
        wgpuRenderPassEncoderSetVertexBuffer(pass, 0, vertexBuffer[i], 0,
                                            vertexDataSize[i]);
        ...
        wgpuRenderPassEncoderDraw(pass, numVertices[i], numInstances[i], 0, 0);
